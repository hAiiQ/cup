...
export default function UserListPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="rounded-lg border border-white/20 bg-black/75 px-8 py-6 text-center backdrop-blur-sm">
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md rounded-lg border border-red-400/40 bg-red-950/80 p-8 text-center backdrop-blur-sm">
  ...
  return (
    <div className="min-h-screen px-4 py-8">
      <div className="mx-auto max-w-7xl">
        <section className="rounded-lg border border-white/20 bg-black/70 p-6 backdrop-blur-sm">
        ...
        {sortedUsers.length === 0 ? (
          <section className="mt-6 rounded-lg border border-white/20 bg-black/70 p-10 text-center backdrop-blur-sm">
          ...
          <>
            <section className="mt-6 hidden overflow-hidden rounded-lg border border-white/20 bg-black/75 backdrop-blur-sm lg:block">
              <table className="w-full table-fixed text-left">
                <thead className="border-b border-white/15 bg-white/10 text-xs uppercase text-white/60">
                  <tr>
                    <th className="w-[12%] px-4 py-3">Username</th>
                    <th className="w-[13%] px-4 py-3">Discord</th>
                    <th className="w-[15%] px-4 py-3">Ingame Name</th>
                    <th className="w-[11%] px-4 py-3">Ingame Rank</th>
                    <th className="w-[11%] px-4 py-3">Peak Rank</th>
                    <th className="w-[6%] px-4 py-3">Lvl</th>
                    <th className="w-[8%] px-4 py-3">Tier</th>
                    <th className="w-[12%] px-4 py-3">Team</th>
                    <th className="w-[12%] px-4 py-3">Teilnahme</th>
                  </tr>
                </thead>
                <tbody>
                  {sortedUsers.map((user) => (
                    <tr key={user.id} className="border-b border-white/10 last:border-b-0">
                      ...
                      <td className="px-4 py-4 text-white/80">{user.discordName || emptyValue}</td>
            ...
            <section className="mt-6 grid gap-4 lg:hidden">
              {sortedUsers.map((user) => (
                <article key={user.id} className="rounded-lg border border-white/20 bg-black/75 p-4 backdrop-blur-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <h2 className="truncate text-xl font-bold text-white">{user.username || emptyValue}</h2>
                      <p className="mt-1 truncate text-sm text-indigo-200">Discord: {user.discordName || emptyValue}</p>
                      <p className="mt-1 truncate text-sm text-white/60">Ingame: {user.inGameName || emptyValue}</p>
                    </div>
